...
import com.learnandroid.foodie.databinding.RecentBuyItemBinding
...
class RecentBuyAdapter(
    ...
    inner class RecentViewHolder(private val binding: RecentBuyItemBinding) :
        ...
        fun bind(position: Int) {
            binding.apply {
                txtFoodName.text = foodNameList[position]
                priceRecentBuy.text = foodPriceList[position]
                txtQuantityRecentBuy.text = foodQuantityList[position].toString()
                val uriString = foodImageList[position]
                val uri = Uri.parse(uriString)
                Glide.with(context).load(uri).into(imgFoodIcon)
            }
        }
    }
}